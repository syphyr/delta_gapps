...
APKLIST=$(find search-lollipop-arm64/optional/apkbin -name com.*.apk | sort)
FILENAME=Velvet.apk
INDIR=search-lollipop-arm64
NAME=gapps-search
VER=arm64-lmn
...
echo "Google Search add-on for 5.0.2+ (arm64) (replaces stock quicsearchbox)" >> build.log
echo "" >> build.log
