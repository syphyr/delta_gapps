...
FILEPATH=$(find chrome-nougat-arm64/system/app -name com.*.apk | sort)
FILENAME=GoogleChrome.apk
INDIR=chrome-nougat-arm64
NAME=gapps-chrome
VER=arm-arm64-n
...
echo "Google Chrome add-on for 7.0.0+ (arm/arm64) (replaces stock web browser) *includes Google Webview*" | tout
echo "" | tout
