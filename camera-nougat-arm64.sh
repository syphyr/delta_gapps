...
APKLIST=$(find camera-nougat-arm64/system/app/GoogleCamera -name com.*.apk | sort)
FILENAME=GoogleCamera.apk
INDIR=camera-nougat-arm64
NAME=gapps-camera
VER=arm64-n
...
echo "Google Camera add-on for 7.1.0+ (arm64) (replaces stock camera)" | tout
echo "" | tout
