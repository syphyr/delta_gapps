...
APKLIST=$(find messenger-arm64/optional/apkbin -name com.*.apk | sort)
FILENAME=GoogleMessenger.apk
INDIR=messenger-arm64
NAME=gapps-messenger
VER=arm64-lmn
...
echo "Google Messenger add-on for 5.0.2+ (arm64) (replaces stock sms/mms application)" | tout
echo "" | tout
