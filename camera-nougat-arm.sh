...
APKLIST=$(find camera-nougat-arm/system/app/GoogleCamera -name com.*.apk | sort)
FILENAME=GoogleCamera.apk
INDIR=camera-nougat-arm
NAME=gapps-camera
VER=arm-n
...
echo "Google Camera add-on for 7.1.0+ (arm) (replaces stock camera)" | tout
echo "" | tout
